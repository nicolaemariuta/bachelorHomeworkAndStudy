...
int f_binarizare( int nivel_gri )
{
    ...
    if( nivel_gri >= 0 && nivel_gri < T ) 
	return	( int )0;
    
    if( nivel_gri >= T && nivel_gri <=255) 
	return	( int ) 255;
...
/*
int f_negativare( int nivel_gri )
{
    int  a = 80;
    int  Va = 20;
    int  b = 170;
    ...
    return	nivel_gri = 255-nivel_gri;
}
*//*
int f_decupare( int nivel_gri )
{
    int  a = 80;
    int  Va = 20;
    int  b = 170;
   ...
   // return	( int ) nivel_gri; cu pastrare de fundal
	return	( int ) 0; //fara pastrare
...
void accentueaza_contrastul( IplImage* image )
{
    ...
    for( i = 0; i < h; i++ )
	     for( j = 0; j < w; j++ )
	     {
	         ...
	         // valoare.val[0] = f_intindere( int( pixel.val[0] ) );
	          valoare.val[0] = f_binarizare( int( pixel.val[0] ) );
	         // valoare.val[0] = f_negativare( int( pixel.val[0] ) );
	         // valoare.val[0] = f_decupare
              ( int( pixel.val[0] ) );
